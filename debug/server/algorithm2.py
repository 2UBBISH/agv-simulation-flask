...
class Node:
    ...
    def __init__(self, robots, cost, conflict):
        self.robots = robots
        self.cost = cost
...
class Algorithm2:
    ...
    def map_init(cls, data):
        ...
        Algorithm2.compute_nodes()
        Algorithm2.compute_edges()
        return {}
    ...
    def compute_route(cls, data):
        ...
        init_node = Node(copy.deepcopy(cls.robots), 0, conflict)
        ...
        while not upper_node.empty():
            ...
            print(r1_id, r2_id)
            # print(prohibit_node, r1_id, r2_id)
            ...
            # 重新规划r2_id机器人
            # print(r1_id, node.conflict.get(prohibit_node))
            ...
            # print(r2_id, temp_node.conflict.get(prohibit_node))
...
class Point(object):
    ...
    def __init__(self, pricost, nowcost, rid):
        self.id = rid
    ...
    def __str__(self):
        return '(' + str(self.pricost) + ', ' + str(self.nowcost) + ', ' + str(self.id) + ')'


def shortest_path(g, conflict, source, target, init_cost, rid):
    priq = PriorityQueue()
    priq.put(Point(init_cost, init_cost, source))
    map_close = {}
    ...
    while not priq.empty():
        ...
        for item in list(g.neighbors(point)):
            ...
            if conflict.get(temp_node) is not None and temp_node != init_node and conflict[
                temp_node] != rid and cost < 68:
                continue
            ...
            for j in range(cost, cost + 6, 4):
                ...
                if map_open.get(item) is None or \
                    ...
                    # 1.5倍误差
                    priq.put(Point(temp + 1.5 * calc_cost(g, item, target), temp, item))
                    map_open[item] = temp
                    fa[item] = point
...
if __name__ == "__main__":
    ...
    f = open("map.json", "r")
    map_data = f.read()
    Algorithm2.map_init(eval(map_data))
    f.close()
    f = open("robot.json", "r")
    map_data = f.read()
    print(map_data)
    Algorithm2.compute_route(eval(map_data))
    f.close()
    end = time.time()
    print(end - start)
