...
class Algorithm3:
    ...
    def map_init(cls, data):
        ...
        Algorithm3.compute_nodes()
        Algorithm3.compute_edges()
        return {}
        ...
        for item in list(g.neighbors(point)):
            ...
            for j in range(cost, cost + 6, 4):
                ...
                if map_open.get(item) is None or \
                    ...
                    priq.put(Point(temp + 1.25 * calc_cost(g, item, target), temp, item))
...
if __name__ == "__main__":
    ...
    f = open("map.json", "r")
    map_data = f.read()
    Algorithm3.map_init(eval(map_data))
    f.close()
    f = open("robot.json", "r")
    map_data = f.read()
    print(map_data)
    Algorithm3.compute_route(eval(map_data))
    f.close()
    end = time.time()
    print(end - start)
